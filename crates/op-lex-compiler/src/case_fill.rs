...
//! hole was filled, but does not itself introduce a verdict. Silence is a
//! property of a successful append. If the host cannot persist the
//! attestation, the fill fails closed.
//!
...
//! evaluates and returns `value`. On successful append this preserves the
//! Op-type of the fill expression as exactly `type_of(value)`, satisfying the
//! §6.3 weak bisimulation equation `[[fill(h, v, w)]] = [[v]]` up to
//! τ-labels on `mu`.
...
pub fn compile_fill(hole_id: &str, value: OpExpr, witness: &Witness) -> OpExpr {
    let attestation_call = OpExpr::Call(
        ...
        vec![
            ("hole_id".to_string(), OpExpr::String(hole_id.to_string())),
            (
                "authority".to_string(),
                OpExpr::String(witness.authority.clone()),
            ),
            ("digest".to_string(), OpExpr::String(witness.digest.clone())),
            (
                "timestamp".to_string(),
