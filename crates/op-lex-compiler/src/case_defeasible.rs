...
pub fn compile_defeasible(
    ...
    // Defeasible lowering: when a rule has at least one exception, this emits
    // `match guard { true -> [[body]]; false -> [[acc]] }`. The exception `body`
    // and the recursed `acc` (the base or a lower-priority exception) are BOTH
    // verdicts of the common `Verdict` type but typically DIFFERENT constructors
    // (e.g. a `NonCompliant` exception over a `Compliant` base). op-core's
    // `OpExpr::Match` result-type rule (in both `check_expr` and
    // `static_expr_type`) computes the least-upper-bound — the constructor-set
    // UNION — of the variant arm types via `join_variant_arm_types` (rejecting
    // only a conflicting payload on a shared tag), so heterogeneous-constructor
    // verdict arms type-check soundly; `infer_expr_type` here mirrors that union
    // so the declared program output matches op-core's synthesized match type.
    // Both the flat (zero-exception) and the heterogeneous defeasible cases
    // compile + run — see the `golden_defeasible_tolling` golden.
